...
notify_sns_lambda=boto3.client("lambda")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        for group in groups_to_add:
            conn.modify(f"CN={group},{base_dn}", {'member': [(ldap3.MODIFY_ADD, [f"CN={user_info['username']},{base_dn}"])]})
        # Add user to the "All Employees" group
        ad_client.add_group_member(
            ClientToken=context.aws_request_id,
            DirectoryId=os.getenv("DIRECTORY_ID"),  # Assuming the Directory ID is set as an environment variable
            GroupName=f"{ROLE_TO_GROUPS_MAP.get(user_info["Role"], [ALL_EMPLOYEES_GROUP])[0]}",  # Add to the specific role group or default to "All Employees"
            MemberName=f"{user_info['username']}@{domain}",
            MemberRealms=[domain]
        )
